    ...
    <div class="py-8 max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-4">
        ...
        @foreach ($reports as $report)
            <div class="bg-white p-4 shadow rounded-lg">
                <p class="text-sm text-gray-500">Article : <a class="text-indigo-600" href="{{ route('posts.show', $report->comment->post) }}">{{ $report->comment->post->title }}</a></p>
                ...
                <form method="POST" action="{{ route('admin.reports.update', $report) }}" class="mt-2 flex gap-2">
                    ...
                    <button class="text-sm text-indigo-600">Mettre à jour</button>
                </form>
            </div>
        @endforeach
        {{ $reports->links() }}

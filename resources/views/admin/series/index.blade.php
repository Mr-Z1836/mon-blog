<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.series.create') }}" class="rounded bg-indigo-600 px-3 py-1 text-white text-sm">Nouvelle série</a>
        </div>
    </x-slot>
        ...
        <div class="bg-white shadow rounded-lg divide-y">
            @foreach ($series as $item)
                <div class="p-4 flex justify-between">
                    ...
                    <a href="{{ route('admin.series.edit', $item) }}" class="text-indigo-600 text-sm">Modifier</a>
                </div>
            @endforeach
        </div>
        {{ $series->links() }}

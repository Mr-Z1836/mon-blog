<div class="{{ $depth > 0 ? 'ms-6 border-s-2 border-slate-200 ps-4 dark:border-slate-700' : '' }} border-b border-slate-200 pb-3 dark:border-slate-700">
    <p class="text-sm font-medium text-slate-900 dark:text-slate-100">
        ...
        @if ($comment->mentionedUser)
            <span class="text-indigo-600">→ @{{ $comment->mentionedUser->username ?? $comment->mentionedUser->name }}</span>
        @endif
    </p>
    <p class="text-sm text-slate-600 dark:text-slate-400">{{ $comment->content }}</p>
    @auth
        <div class="mt-2 flex gap-3 text-xs">
            <button type="button" class="text-indigo-600 reply-btn" data-parent="{{ $comment->id }}">Répondre</button>
            <form method="POST" action="{{ route('posts.comments.report', [$post, $comment]) }}" class="inline" onsubmit="return confirm('Signaler ce commentaire ?')">
                ...
                <button type="submit" class="text-rose-600">Signaler</button>
            </form>
        </div>
    @endauth
